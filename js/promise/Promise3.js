var Promise = (function() {
  function Promise(executor) {
    ...
    // ensure using Promise like this: new Promise(executor);
    // not a nornal function calling: Promise();
    if (!(this instanceof Promise)) return new Promise(executor);

    var self = this;
    self.callbacks = []; // onResolved, onRejected callbacks when Promise done
    self.status = 'pending'; // initialized status

    function resolve(value) {
      setTimeout(function() {
        if (self.status !== Promise.STATUS_PENDING) {
          return;
        }
        self.status = Promise.STATUS_RESOLVED;
        self.data = value;
    ...
    function reject(reason) {
      setTimeout(function(){
        ...
        self.status = Promise.STATUS_REJECTED;
        self.data = reason;
    ...
    try{
      ...
      reject(e); // user executor error occurs
    }
  }

  Promise.STATUS_PENDING = 'pending';
  Promise.STATUS_RESOLVED = 'resolved';
  Promise.STATUS_REJECTED = 'rejected';
